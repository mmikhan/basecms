...
export const ImageMedia: React.FC<MediaProps> = ({
  ...
  const [loader] = useState<typeof loading>(loading ?? (!priority ? 'lazy' : undefined))
  const [width, setWidth] = useState<number | undefined>()
  const [height, setHeight] = useState<number | undefined>()
  const [imageSrc, setImageSrc] = useState<StaticImageData | string | null>(src ?? null)
  const [imageAlt, setImageAlt] = useState<typeof alt>(alt)
  ...
  useEffect(() => {
    if (!src && resource && typeof resource === 'object') {
      ...
      setWidth(fullWidth ?? undefined)
      setHeight(fullHeight ?? undefined)
      setImageAlt(altFromResource ?? undefined)

      const cacheTag = resource.updatedAt
      setImageSrc(getMediaUrl(url, cacheTag))
    }
  }, [src, resource])
